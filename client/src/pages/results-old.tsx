...
export default function Results() {
  ...
  // Helper functions for contact display (no longer needed with ContactTable)
  ...
  const getConfidenceBadge = (confidence: number | null) => {
    if (!confidence) return { variant: "secondary", className: "", text: "Unknown", tooltip: "No confidence score available" };
    ...
    if (confidence >= 90) {
      return {
        ...
        tooltip: `Very high confidence (${confidence}%) - Strong match based on title, department, and company context`
      };
      ...
      return {
        ...
        text: `${confidence}% Match`,
        tooltip: `Good confidence (${confidence}%) - Likely match with some uncertainty`
      };
    } else {
      return {
        variant: "secondary",
        ...
        text: `${confidence}% Match`,
        tooltip: `Lower confidence (${confidence}%) - Possible match, verify details`
      };
    }
  };
  ...
  const getVerificationBadge = (status: string, isVerified: boolean) => {
    if (status === "valid" && isVerified) {
      return {
        ...
        text: "✅ Verified",
        tooltip: "Email verified through multiple sources"
      };
      ...
      return {
        ...
        className: "border-yellow-500 text-yellow-700",
        text: "⚠️ Risky",
        tooltip: "Email exists but may have delivery issues"
      };
      ...
      return {
        ...
        className: "",
        text: "❌ Invalid",
        tooltip: "Email address is invalid or non-existent"
      };
    } else {
      return {
        variant: "secondary",
        className: "",
        text: "Unverified",
        tooltip: "Email verification status unknown"
      };
    }
  };
